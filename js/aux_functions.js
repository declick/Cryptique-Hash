...
function LoadData(needGenerateSalt) {
    ...
    let info = {
        complexNum: 1, // Complexité fixée à 1
...
function MakeShufflePositionArr(hash, messageLength) {
    ...
    //L'enregistrement a échoué Pour des raisons d'évolutivité, ici le code est découpé en tableaux ARR_SIZE.
    //Chaque tableau sera mélangé à l'intérieur de lui-même, donc sera réuni.
    //pourquoi sqrt de messageLength? parce que le temps pour résoudre un arr est size^2, donc, si vous avez N arr complet, et une position arr size = messageLength/N ---> time = (mL/N)^2 + N^2 ---> ... ---> best solution = sqrt(mL)
    ...
    //Transformer le vecteur fullArr en une matrice de tableaux
    for (let i = 0; i < NUMBER_OF_FULL_ARR; i++) fullArrs[i] = [];


    //Passer les valeurs aux tableaux
    ...
    //Créer un tableau pour ordonner le shuffle concat
    let positionArr = [];
    ...
    //Mélangez les tableaux et concat
...
function HashComplex(originalHash, complexity) {
    ...
    //Cette fonction fait le hachage encore et encore. Faire le hachage à partir d'un hachage.
    //Le nombre de fois que le hachage est effectué est exponentiel (10 ^ complexité ---> i < Math.Pow(10, complexité) - 1).
    //Cette fonction augmente le temps de résolution de la cryptographie par l'entrée "complexité".
    // Par cette fonction, le temps disponible pour un ordinateur 2022 normal résout un petit message avec un mot de passe long de 6 et un nombre de complexité 5 est de 3200 ms.
    // Cela signifie que si un mot de passe standard de six caractères composé uniquement de chiffres et de lettres minuscules/majuscules - 62 caractères possibles - a 62 ^ 6 possibilités.
    //Et avec un sel de 4 caractères - 62^4 possibilités de sel
    //Le temps pour résoudre tous les messages possibles - et ensuite résoudre la cryptographie - par la force brute est de (62^4) x (62^6) x 3200 ms
    //(62^4) x (62^6) x 3200 ms = 2 685 757 970 778 688 716 800 ms = 31 085 161 698 827 jours = 85,16 milliards d'années
    //*Ce calcul ne compte pas le temps nécessaire pour valider une solution (augmente le temps pour trouver une solution correcte).
    //* Et considérez simplement un type linéaire de solveur. Un solveur plus efficace essaiera de tester les mots de passe les plus courants pour réduire le temps nécessaire.

    let hashOutput = originalHash;
    ...
    //problème : Erreur de dépassement de la taille maximale de la pile d'appels
...
// Prise en charge de la sortie ArrayBuffer
let arrayBuffer = sha512.arrayBuffer('Message to hash');

// Prise en charge de la sortie du tableau
let bytes = sha512.digest('Message to hash');
let bytes = sha512.array('Message to hash');
...
let hash = sha512.hmac.create('key');
hash.update('Message to hash');
hash.hex();

let hash2 = sha512.hmac.update('key', 'Message to hash');
...
(function () {
    ...
    let INPUT_ERROR = 'entrée de type invalide';
    ...
    let NODE_JS = !root.JS_SHA512_NO_NODE_JS && typeof process === 'object' && process.versions && process.versions.node;
    if (NODE_JS) {
        root = global;
    } else if (WEB_WORKER) {
        root = self;
    }
    let COMMON_JS = !root.JS_SHA512_NO_COMMON_JS && typeof module === 'object' && module.exports;
    let AMD = typeof define === 'function' && define.amd;
    let ARRAY_BUFFER = !root.JS_SHA512_NO_ARRAY_BUFFER && typeof ArrayBuffer !== 'undefined';
    ...
    HmacSha512.prototype.clone = function () {
        let hash = new HmacSha512([], this.bits, false);
        this.copyTo(hash);
